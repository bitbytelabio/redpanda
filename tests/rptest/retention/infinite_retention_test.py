...
class InfiniteRetentionTest(PreallocNodesTest):
    ...
    # Calculate other
    # bitrates
    produce_byte_rate = 20 * 1024 * 1024
    consume_byte_rate = 20 * 1024 * 1024
    ...
    def long_retention_with_spillover_test(self):
        ...
        # seq_consumer = self._create_seq_consumer()
        group_consumer = self._create_group_consumer()
        ...
        while iteration <= iteration_count:
            ...
            # wait and stop
            producer.wait(timeout_sec=self.expected_producer_duration)
            # rand_consumer.wait(timeout_sec=self.expected_consumer_duration)
            # seq_consumer.wait(timeout_sec=self.expected_consumer_duration)
            group_consumer.wait(timeout_sec=self.expected_consumer_duration)
            ...
            for k, v in iteration_timings.items():
                self.logger.info(f"{k}={v[iteration]}s")
            if scrub_exception:
                break
            else:
                # do next iteration
        ...
        for k, v in iteration_timings.items():
            line = f"{k:<18}"
            for idx in range(1, iteration_count+1):
                if k in ["high_watermark", "manifest_uploads", "segment_uploads"]:
                    _t = f"{v[idx]}"
                else:
                    _t = f"{v[idx]:.2f}s"
                line += f"{_t:^14}"
